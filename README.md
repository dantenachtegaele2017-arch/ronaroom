...
- [x] Real icon system: colored "badge" containers (rounded-square, consistent size, color-coded per category) with a symbol centered inside, instead of bare inline emoji — same pattern real apps use for icons, applied to every section header and dashboard stat. (Checked Adobe Stock for a proper icon set first — it's built for photography/illustration, not small transparent UI icons, so a custom badge system was the more reliable path; flagging this so future icon requests default straight to the badge system rather than re-trying stock search.)
- [x] Upgrades page rebuilt as three color-coded tech trees — Capabilities (violet), Compute (blue), Power (amber) — each card border-accented in its tree's color, which also matches the color of the Dashboard stat that tree grows (Energy=amber matches Power, Server capacity=blue matches Compute, Model strength=violet matches Capabilities)
- [x] Fog of war on all three trees: you only see what's unlocked plus exactly one next locked item (with its real name/cost, since you need to know what to save toward) — everything further down the tree is fully hidden behind a "🔒 N more beyond this one" placeholder, not just labeled "Locked"
- [x] Compute/Power tiers now unlock progressively too (previously all 5 tiers were simultaneously visible/buyable from the start) — a tier only reveals once the previous one has been bought at least once, matching the same fog-of-war rule as Capabilities
- [x] **New: confirmation dialog before "Start new game"** — previously one misclick permanently erased all progress with no way back; now it asks first
- [x] **New: offline progress** — previously closing the game froze it completely, which is backwards for an idle game; now reopening after a gap fast-forwards the simulation (capped at 300 in-game days per session, so a long absence gives a welcome-back boost without trivializing the run) and shows a summary notification
- [x] **New: a proper win screen** — previously "winning" just changed one line of text buried in a scrolled panel; now it's a full-screen summary (launch date → completion date, days elapsed, final model strength, capabilities trained) with a "Launch a new model" button
...
Savegame location on Windows: `%APPDATA%\Godot\app_userdata\CousinGame\savegame.json`. Delete that file (or use the "Start new game" button in-game, now behind a confirmation) to reset. v11 saves stay compatible with v9/v10 saves — new fields (offline-progress timestamp) just default sensibly if missing.

## Game logic

Everything lives in `scripts/Main.gd` (UI is built in code, no separate `.tscn` nodes, to avoid merge conflicts). Key tunable data sits near the top of the script: `SEGMENT_DATA` (population per segment), `capabilities` (data cost + training energy + permanent growth multiplier per capability), `MAX_TRAINING_ENERGY_RATE` (training speed cap), `MAX_OFFLINE_DAYS` (offline-progress cap), `base_growth_rate`, `revenue_per_user`/`data_per_user`, `compute_tiers`/`power_tiers` (name/cost/growth/output per tier), `BASE_SERVING_CAPACITY`/`BASE_ENERGY_REGEN`/`BASE_ENERGY_CAPACITY` (free baseline before buying anything), and the `COLOR_*` constants (per-tree accent colors). Adjust those to test balance. The globe texture is `assets/sprites/world_map.jpg` (licensed via Adobe Stock). The `_simulate_tick()` function is the single source of truth for one game-day of simulation — both live play and offline catch-up call it, so any economy change only needs to happen in one place.
